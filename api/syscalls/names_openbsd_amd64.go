...
var Name = map[uint64]string{
	...
	unix.SYS_GETENTROPY:     "getentropy",     // { int sys_getentropy(void *buf, size_t nbyte); }
	...
	unix.SYS_ACCESS:         "access",         // { int sys_access(const char *path, int amode); }
	...
	unix.SYS_READLINK:       "readlink",       // { ssize_t sys_readlink(const char *path, \
	...
	unix.SYS_SETPGID:        "setpgid",        // { int sys_setpgid(pid_t pid, pid_t pgid); }
	unix.SYS_FUTEX:          "futex",          // { int sys_futex(uint32_t *f, int op, int val, \
	unix.SYS_UTIMENSAT:      "utimensat",      // { int sys_utimensat(int fd, const char *path, \
	unix.SYS_FUTIMENS:       "futimens",       // { int sys_futimens(int fd, \
	unix.SYS_KBIND:          "kbind",          // { int sys_kbind(const struct __kbind *param, \
	...
	unix.SYS_ACCEPT4:        "accept4",        // { int sys_accept4(int s, struct sockaddr *name, \
	...
	unix.SYS_PIPE2:          "pipe2",          // { int sys_pipe2(int *fdp, int flags); }
	unix.SYS_DUP3:           "dup3",           // { int sys_dup3(int from, int to, int flags); }
	...
	unix.SYS_CHFLAGSAT:      "chflagsat",      // { int sys_chflagsat(int fd, const char *path, \
	unix.SYS_PLEDGE:         "pledge",         // { int sys_pledge(const char *promises, \
	...
	unix.SYS_SENDSYSLOG:     "sendsyslog",     // { int sys_sendsyslog(const char *buf, size_t nbyte, \
	unix.SYS_GETSOCKOPT:     "getsockopt",     // { int sys_getsockopt(int s, int level, int name, \
	unix.SYS_THRKILL:        "thrkill",        // { int sys_thrkill(pid_t tid, int signum, void *tcb); }
	...
	unix.SYS_GETLOGIN_R:     "getlogin_r",     // { int sys_getlogin_r(char *namebuf, u_int namelen); }
	...
	unix.SYS_SYSCTL:         "sysctl",         // { int sys_sysctl(const int *name, u_int namelen, \
